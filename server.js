...
app.get('/amo-leads-test',async(_,res)=>{try{const data=await amo('/api/v4/leads?limit=5');const leads=data?._embedded?.leads||[];res.json({amo_connected:true,leads_received:leads.length,leads:leads.map(l=>({id:l.id,name:l.name,status_id:l.status_id,responsible_user_id:l.responsible_user_id,price:l.price}))});}catch(e){res.status(500).json({amo_connected:false,error:e.message});}});app.get('/amo-stats',async(_,res)=>{try{const [leads,tasks,users,pipelines]=await Promise.all([all('/api/v4/leads','leads'),all('/api/v4/tasks','tasks'),all('/api/v4/users','users'),all('/api/v4/leads/pipelines','pipelines')]);res.json({amo_connected:true,leads:leads.length,tasks:tasks.length,users:users.length,pipelines:pipelines.length});}catch(e){res.status(500).json({amo_connected:false,error:e.message});}});app.get('/api/users',async(_,res)=>{try{res.json(await all('/api/v4/users','users'));}catch(e){res.status(500).json({error:e.message});}});
