...
app.get('/amo-stats',async(_,res)=>{try{const [leads,tasks,users,pipelines]=await Promise.all([all('/api/v4/leads','leads'),all('/api/v4/tasks','tasks'),all('/api/v4/users','users'),all('/api/v4/leads/pipelines','pipelines')]);res.json({amo_connected:true,leads:leads.length,tasks:tasks.length,users:users.length,pipelines:pipelines.length});}catch(e){res.status(500).json({amo_connected:false,error:e.message});}});app.get('/api/users',async(_,res)=>{try{res.json(await all('/api/v4/users','users'));}catch(e){res.status(500).json({error:e.message});}});
